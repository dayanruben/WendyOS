...
Managed startup now passes on WendyOS nightly-20260908T175754 using verified legacy IPv4/IPv6 isolation and scoped host-module preparation by the updated agent. Named-VM agent update and reboot followed by robot readiness pass manual checks. App/terminal independence, crash recovery, profile continuity, concurrent inspection, and two-VM/port isolation still need complete lifecycle acceptance. The matching checkout agent is required until a supporting release is available; the cached base-image agent does not advertise `go2-virtual-robot`.

A clean local producer benchmark with both cameras, 3D lidar and native state measures 500 Hz physics/LowState, 200 Hz IMU, 50 Hz policy, 10 Hz lidar and 14.59 fps; it does not include external DDS reader load. Full-subscriber VM diagnostics on four and six vCPUs failed camera throughput/freshness targets. A later four-vCPU run failed camera source freshness after 11.1 seconds (556.6 ms age); its render-stage snapshot was captured after subscriber exit. **The ten-minute VM gate has not passed.** Recorded results and their distinct scopes are in [`validation/`](../../simulator/go2/validation/). Factory firmware parity, native video, depth and deterministic ROS-time mode remain outside the implemented profile.
