...
Date: 2026-09-12. Status: implementation in progress. The sequence below defines acceptance gates; targets are not results unless recorded in the implementation status.

Implementation status: the runtime, standard ROS loop, native Unitree subset, sensor sandbox, managed profile and typed inspection integration are implemented in [`simulator/go2`](../../simulator/go2/README.md). A separate deployed VM app has passed signed motion, sensor, expiry and reset checks. Actual pinned SDK/stock ROS interoperability and a separate navigation app's goal/obstacle/stale-scan scenarios have passed in isolated Linux containers. Camera/lidar fault controls pass real ROS lifecycle tests. A clean local producer benchmark with both cameras, 3D lidar and native state measures 500 Hz physics/LowState, 200 Hz IMU, 50 Hz policy, 10 Hz lidar and 14.59 fps; it does not include external DDS reader load. The ten-minute VM test with full subscribers and managed lifecycle acceptance remain in progress. Recorded results and their scope are in [`validation/`](../../simulator/go2/validation/). Factory firmware parity, native video, depth and deterministic ROS-time mode remain outside the implemented profile.
...
The current app inspector forces localhost-only discovery, which is compatible with this loopback topology. It only sources the base ROS setup file, so having a custom overlay in an image is not enough. Add an explicit validated setup path for the managed simulation image, scoped sidecar identity/routing, and tests with another ROS app running concurrently.
