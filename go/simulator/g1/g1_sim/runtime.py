...
class Runtime:
    def __init__(self, *, render=True, simulation=None, ros=False):
        self.source_digest = os.environ.get("G1_SOURCE_DIGEST", "")
        self.world = os.environ.get("G1_WORLD", "indoor")
        if self.world != "indoor":
            raise ValueError("this G1 profile supports the indoor world")
        self.seed = int(os.environ.get("G1_SEED", "0"))
        if not 0 <= self.seed <= 0xffffffff:
            raise ValueError("G1_SEED must be an unsigned 32-bit integer")
        ...
        self.scene = BrowserScene(self.sim.model)
    ...
    def _render(self):
        ...
        try:
            ...
            detail = os.environ.get("G1_VISUAL_DETAIL", "full")
            if detail not in {"full", "balanced"}:
                raise ValueError("G1_VISUAL_DETAIL must be full or balanced")
            # Both G1 settings retain the pinned model meshes. Balanced removes
            # expensive camera effects; browser orbit rendering runs on the host.
            ...
            while not self.stop_event.is_set():
                ...
                if emit_camera:
                    ...
                    renderer.scene.flags[mujoco.mjtRndFlag.mjRND_SHADOW] = detail == "full"
                    renderer.scene.flags[mujoco.mjtRndFlag.mjRND_REFLECTION] = detail == "full"
    ...
    def status(self):
        ...
        result.update({
            "robot": "g1", "simulation": True, "profile_version": 1,
            "robot_kind": "g1", "source_digest": self.source_digest,
            "vm_name": os.environ.get("G1_VM_NAME", ""),
            "dds_isolation": os.environ.get("G1_ISOLATION_STATUS", "unmanaged"),
            "policy_bundle": self.policy_bundle, "world": self.world,
            "clock_mode": "device", "seed": self.seed,
            "visual_detail": os.environ.get("G1_VISUAL_DETAIL", "full"),
            "healthy": healthy,
