...
func TestSourceDigestCanonical(t *testing.T) {
	...
	want := "sha256:a6bc8964ccb19340d46c5e4faabe6abddbea0964ed9bf73a993b5ed8ca34eaf1"
...
func TestEmbeddedSourcesAllowlist(t *testing.T) {
	want := []string{
		...
		"ros_ws/src/go2_command_ingress/src/command_ingress.cpp",
		"tools/build_visuals.py", "tools/fetch_assets.py", "tools/fetch_unitree.py", "tools/prepare_unitree.py",
		"unitree.lock.json", "visuals.lock.json", "wendy.json",
	}
	slices.Sort(want)
...
func TestMaterializeIdentity(t *testing.T) {
	...
	want := filepath.Join(root, "go2", strings.TrimPrefix(SourceDigest(), "sha256:"))
	...
	for _, modification := range []string{"content", "missing", "extra-file", "extra-directory", "file-symlink", "directory-symlink", "root-symlink"} {
		t.Run(modification, func(t *testing.T) {
			...
			case "directory-symlink":
				err = os.Symlink("go2_sim", filepath.Join(path, "extra-link"))
			case "root-symlink":
...
func TestMaterializeConcurrent(t *testing.T) {
	...
	entries, err := os.ReadDir(filepath.Join(root, "go2"))
	if err != nil {
		t.Fatal(err)
	}
	if len(entries) != 2 {
...
func TestMaterializeCleansOnlyOwnedTemporary(t *testing.T) {
	root := cacheRoot(t)
	foreign := filepath.Join(root, "go2", ".materialize-foreign")
...
func TestMaterializeRejectsInvalidRoot(t *testing.T) {
	...
	root := cacheRoot(t)
	if err := os.WriteFile(filepath.Join(root, "go2"), []byte("user data"), 0o600); err != nil {
		t.Fatal(err)
	}
	if _, err := Materialize(root); err == nil {
		t.Fatal("file used as cache directory")
	}
	if got, err := os.ReadFile(filepath.Join(root, "go2")); err != nil || string(got) != "user data" {
