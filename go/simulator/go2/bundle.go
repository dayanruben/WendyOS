// Package go2 provides the authored source payload for the virtual Go2 runtime.
...
//go:embed tools/fetch_assets.py tools/fetch_unitree.py tools/build_visuals.py tools/prepare_unitree.py
...
var sourceDigest = sync.OnceValue(func() string {
	...
	if err != nil {
		...
		panic(fmt.Sprintf("invalid embedded Go2 source bundle: %v", err))
	}
	return digest
})
...
// below cacheRoot/go2. The published tree is read-only and must not be edited.
...
func materialize(cacheRoot string, tree fs.FS, digest string) (string, error) {
	if strings.TrimSpace(cacheRoot) == "" {
		return "", errors.New("Go2 source cache root is empty")
	}
	root, err := filepath.Abs(cacheRoot)
	if err != nil {
		return "", fmt.Errorf("resolving Go2 source cache: %w", err)
	}
	root = filepath.Join(root, "go2")
	if err := os.MkdirAll(root, 0o700); err != nil {
		return "", fmt.Errorf("creating Go2 source cache: %w", err)
	}
	...
	if _, err := os.Lstat(target); err == nil {
		...
		return "", fmt.Errorf("inspecting Go2 source cache: %w", err)
	}

	temporary, err := os.MkdirTemp(root, ".materialize-")
	if err != nil {
		return "", fmt.Errorf("creating temporary Go2 source directory: %w", err)
	}
	...
	if err := writeSources(tree, temporary); err != nil {
		return "", fmt.Errorf("materializing Go2 sources: %w", err)
	}
	if err := verifyCache(temporary, digest); err != nil {
		return "", err
	}
	if err := makeReadOnly(temporary); err != nil {
		return "", fmt.Errorf("protecting Go2 source cache: %w", err)
	}
	...
	if err := os.Rename(temporary, target); err != nil {
		return "", fmt.Errorf("publishing Go2 source cache: %w", err)
	}
...
func digestTree(tree fs.FS) (string, error) {
	...
	h := sha256.New()
	h.Write([]byte("wendy-go2-source-v1\x00"))
	var length [8]byte
...
func requireDirectory(path string) error {
	...
	if !info.IsDir() {
		return fmt.Errorf("Go2 source cache is not a directory: %s", path)
	}
	return nil
}
...
func verifyCache(path, expected string) error {
	if err := requireDirectory(path); err != nil {
		return fmt.Errorf("invalid Go2 source cache %q: %w", path, err)
	}
	...
	if digest != expected {
		return fmt.Errorf("Go2 source cache %q was modified: expected %s, got %s", path, expected, digest)
	}
	return nil
}
...
func lockCache(root string) (func(), error) {
	...
	if info, err := os.Lstat(path); err == nil && !info.Mode().IsRegular() {
		return nil, fmt.Errorf("Go2 cache lock is not a regular file: %s", path)
	} else if err != nil && !errors.Is(err, os.ErrNotExist) {
		return nil, err
	}
	f, err := os.OpenFile(path, os.O_CREATE|os.O_RDWR, 0o600)
	if err != nil {
		return nil, fmt.Errorf("opening Go2 cache lock: %w", err)
	}
	...
	for {
		...
		if err != nil || time.Now().After(deadline) {
			f.Close()
			if err != nil {
				return nil, fmt.Errorf("locking Go2 source cache: %w", err)
			}
			return nil, errors.New("timed out waiting for Go2 source cache lock")
		}
		if locked {
