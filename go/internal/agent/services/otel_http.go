...
const (
	...
	maxOTELHTTPCompressedBodySize = 512 * 1024       // 512 KB compressed — legitimate OTLP protobuf compresses ≥20:1
)
...
func (r *OTELHTTPReceiver) readBody(req *http.Request) ([]byte, error) {
	...
	if isGzipEncoded(req) {
		...
		compressedLimit := io.LimitReader(req.Body, maxOTELHTTPCompressedBodySize+1)
