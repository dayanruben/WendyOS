...
type ApplyOptions struct {
	...
	// Logger receives audit events for security-relevant entitlement
	// decisions (e.g. which NVIDIA device nodes the GPU entitlement
	// discovered and granted). nil is safe: events are dropped.
	Logger *zap.Logger
}

// logger returns the configured audit logger, or a no-op logger when unset,
// so entitlement code never has to nil-check.
func (o ApplyOptions) logger() *zap.Logger {
	if o.Logger != nil {
		return o.Logger
	}
	return zap.NewNop()
...
func applyGPU(spec *Spec, logger *zap.Logger) {
	...
	nodes := discoverNvidiaDeviceNodes(logger)
...
// matches (e.g. the /dev/nvidia-caps directory itself) and stat errors are
// skipped silently — a host without the driver loaded simply returns nothing
// — but a real character device whose name is not on the NVIDIA allowlist is
// skipped with a warning.
func discoverNvidiaDeviceNodes(logger *zap.Logger) []nvidiaDeviceNode {
	...
	for _, glob := range nvidiaDeviceGlobs {
		...
		for _, p := range matches {
			...
			if !nvidiaDeviceNameRe.MatchString(filepath.Base(p)) {
				logger.Warn("GPU entitlement: ignoring character device with unexpected name",
					zap.String("path", p), zap.Int64("major", major), zap.Int64("minor", minor))
				continue
			}
