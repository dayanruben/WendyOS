...
// plugins/main/bridge/bridge.go. Upstream ships this as `package main`
// (a standalone CNI plugin binary), which cannot be imported directly, so
// the source is copied here with the package renamed and its `func main()`
// removed. The command functions (renamed cmdAdd/cmdCheck/cmdDel/cmdStatus ->
// CmdAdd/CmdCheck/CmdDel/CmdStatus) are exported so cmd/wendy-agent can
// dispatch to them directly when the agent
// self-execs as its own CNI bridge plugin (see internal/agent/containerd/cni.go
// and cmd/wendy-agent's CNI dispatch files) instead of shelling out to a
// third-party /opt/cni/bin/bridge binary.
//
// Keep this file in sync with upstream on CNI plugin version bumps: re-copy
// plugins/main/bridge/bridge.go, re-apply the package rename + main() removal
// + exports below, and diff against the previous vendored copy to review
// upstream behavioural changes.
