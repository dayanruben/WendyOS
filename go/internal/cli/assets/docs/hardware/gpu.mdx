...
On **Qualcomm Dragonwing** boards (such as the IQ-8275), the entitlement exposes the Adreno GPU's DRM render node, which the mesa freedreno/turnip, OpenCL, and Vulkan userspace in your image opens. The on-SoC Hexagon NPU is a separate accelerator: `wendy device info` reports it on its own `NPU:` line as `qualcomm (qnn)` when its FastRPC nodes are present — never as an Adreno compute backend — and apps reach it through the `npu` entitlement rather than `gpu`.
