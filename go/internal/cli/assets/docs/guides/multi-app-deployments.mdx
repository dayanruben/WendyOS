...
- A project directory containing either a `wendy.json` with a `services` map, or a `docker-compose.yml` (optionally with a companion `wendy.json`)

<Callout type="warning">
Multi-service projects target Linux/WendyOS devices. Wendy for Mac does not run multi-container stacks — `wendy run` rejects them before any build when the selected target is a Mac.
</Callout>
