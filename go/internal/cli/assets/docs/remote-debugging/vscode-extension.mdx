...
The **Operating System Cache** panel lets you browse cached images, delete old ones to free space, and reveal the cache directory in your file manager. The cache lives under the platform cache directory: `~/Library/Caches/wendy/os-images` (macOS), `~/.cache/wendy/os-images` (Linux, or `$XDG_CACHE_HOME/wendy/os-images`), `%LocalAppData%\wendy\os-images` (Windows). Note this is *not* `~/.wendy`, which holds configuration.
