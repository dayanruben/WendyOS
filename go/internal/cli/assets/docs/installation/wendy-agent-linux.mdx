...
`wendy-agent` comes pre-installed on WendyOS, which currently supports [NVIDIA Jetson Orin Nano and AGX Orin](/docs/installation/wendyos-nvidia-jetson) plus [Raspberry Pi 3, 4, and 5](/docs/installation/wendyos-raspberry-pi-5). If you flashed WendyOS onto your device, you do not need to follow this guide.
...
For Apple Silicon Mac targets, use [Wendy for Mac](/docs/installation/wendy-agent-macos). It follows the same `wendy-agent` model, but uses a macOS-specific app instead of this Linux install script.
