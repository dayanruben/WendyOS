...
Headless Mac lets you use any Apple Silicon Mac as a Wendy target. Install Wendy Agent on the Mac, point the CLI at it, and deploy native macOS apps the same way you deploy to a Jetson or Raspberry Pi — with `wendy run`.

Apps run as native macOS processes, not Linux containers. This makes Headless Mac a natural fit for:
