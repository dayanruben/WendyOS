...
to the target unless `wendy.json` explicitly references it. Homebrew must already
be installed on the target Mac; Wendy reports a clear error if `brew` is missing
or if `brew bundle` fails. This only applies to native Mac deployments.
