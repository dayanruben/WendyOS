---
title: NVIDIA Jetson
description: Install WendyOS on your NVIDIA Jetson device
---

<div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1.5rem' }}>
  <img src="/icons/icons8-nvidia.svg" alt="NVIDIA" style={{ width: '48px', height: '48px' }} />
  <h1 style={{ fontSize: '2.25rem', fontWeight: 'bold', margin: 0, lineHeight: '1' }}>NVIDIA Jetson Installation</h1>
</div>

Jetson Orin Nano, Jetson AGX Orin, and Jetson AGX Thor install through USB recovery. Orin recovery updates QSPI and the selected NVMe/eMMC together; an NVMe-to-USB adapter is needed only for explicit `--rootfs-only` imaging.

## Installing WendyOS on NVIDIA Jetson

WendyOS supports these NVIDIA Jetson devices:

<div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.75rem', margin: '1rem 0' }}>
  <span style={{ border: '1px solid var(--border, #e5e7eb)', borderRadius: '9999px', padding: '0.35rem 0.9rem', fontWeight: 600 }}>✅ Jetson Orin Nano</span>
  <span style={{ border: '1px solid var(--border, #e5e7eb)', borderRadius: '9999px', padding: '0.35rem 0.9rem', fontWeight: 600 }}>✅ Jetson AGX Orin</span>
  <span style={{ border: '1px solid var(--border, #e5e7eb)', borderRadius: '9999px', padding: '0.35rem 0.9rem', fontWeight: 600 }}>✅ Jetson AGX Thor</span>
</div>

For DGX Spark, use [Wendy Agent — Linux](/docs/installation/wendy-agent-linux) on the existing OS instead of flashing WendyOS. Prefer video? Watch the [Jetson Orin Nano installation walkthrough](https://youtu.be/TtDJAtBUEZI).

### Installation Steps

<Steps>
  <Step>
    ### Prepare Storage or Recovery Mode

    <Tabs items={['Orin Nano / AGX Orin', 'AGX Thor']} groupId="jetson-model" persist>
      <Tab value="Orin Nano / AGX Orin">
        Start with the devkit powered off. Connect the host to its recovery/device-mode USB-C port, enter Force Recovery mode, and leave the target NVMe installed. AGX Orin can instead select onboard eMMC.

        <video width="100%" autoPlay loop muted playsInline>
          <source src="/videos/installation/jetson-nvme-mac-adapter.mp4" type="video/mp4" />
          Your browser does not support the video tag.
        </video>

        Full Orin recovery is supported on macOS and Linux. It erases QSPI and all data on the selected NVMe/eMMC, including `/data`.
      </Tab>
      <Tab value="AGX Thor">
        Start with the Thor powered off. Connect your computer to the USB-C port next to the HDMI port, then enter recovery mode: plug in power, hold **Force Recovery**, briefly tap **Reset**, then release **Force Recovery**. The other USB-C port is power-only.
      </Tab>
    </Tabs>
  </Step>

  <Step>
    ### Install WendyOS

    Use the `wendy` CLI to install the operating system:

    ```bash
    wendy install
    ```

    <CliShot flow="os-install" step="select-device-type" alt="The Wendy CLI prompting you to choose a device type before flashing WendyOS" />

    Select the Jetson model you are installing:

    - **Jetson Orin Nano**: the CLI verifies the exact devkit identity, then recovers QSPI plus the installed NVMe.
    - **Jetson AGX Orin**: choose NVMe or eMMC; noninteractive use requires `--storage nvme` or `--storage emmc`.
    - **Jetson AGX Thor**: the CLI downloads a Thor flashpack, scans for the board in USB recovery mode, and flashes QSPI plus the internal NVMe. No external drive is selected.

    The CLI also offers optional first-boot provisioning — joining WiFi, applying critical updates, and pre-enrolling the device to Wendy Cloud. Each can also be set up after the device boots. See [`wendy install`](/docs/advanced/clients/wendy-cli/commands/os/install) for more.
  </Step>

  <Step>
    ### Install Storage or Reboot

    <Tabs items={['Orin Nano / AGX Orin', 'AGX Thor']} groupId="jetson-model" persist>
      <Tab value="Orin Nano / AGX Orin">
        After the CLI has collected the final `SUCCESS` status, let the Jetson reboot. Do not disconnect it while the CLI is waiting for final device status.

        To preserve the old raw-media workflow instead, pass `--rootfs-only`. This does not update QSPI and is not available for eMMC.
      </Tab>
      <Tab value="AGX Thor">
        After the CLI reports a successful flash, power-cycle the Thor out of recovery mode so it boots WendyOS from its internal storage.
      </Tab>
    </Tabs>
  </Step>

  <Step>
    ### Connect the Jetson to Your Computer

    Connect your Jetson to your computer using the data-capable USB-C port. This will allow you to deploy and manage apps on your device.

    <video width="100%" autoPlay loop muted playsInline>
      <source src="/videos/installation/jetson-mac-usb-cable.mp4" type="video/mp4" />
      Your browser does not support the video tag.
    </video>
  </Step>

  <Step>
    ### Power On the Device

    With the supplied power supply, insert the power into the power slot on your Jetson and then plug into a power source like your wall outlet.

    <video width="100%" autoPlay loop muted playsInline>
      <source src="/videos/installation/jetson-power-cable-insertion.mp4" type="video/mp4" />
      Your browser does not support the video tag.
    </video>
  </Step>

  <Step>
    ### Discover Your Device

    Wait 1-2 minutes after powering on your Jetson to allow WendyOS to fully boot. Then run:

    ```bash
    wendy discover
    ```

    You should see your Jetson appear in the list of discovered devices.

    <CliShot flow="discover" step="devices" alt="The Wendy CLI listing discovered WendyOS devices on the network" />

    Your device is now ready for development! For more information on device discovery and configuration, see [Discovering Devices](/docs/device/discovering-devices).
  </Step>
</Steps>

### Next Steps

Now that WendyOS is installed, you can start deploying apps to your device. Check out the [Hello World Guide](/docs/guides/tutorials/python/hello-world) to build your first app.
