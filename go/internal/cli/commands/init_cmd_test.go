...
func TestTemplateTargetMatch_AcceptsExplicitDarwinTarget(t *testing.T) {
	tmpl := repoMetaTemplate{Name: "mlx-openwebui", Targets: []string{targetDarwin}}
...
func TestResolveTemplateLanguage_DarwinRequiresSwift(t *testing.T) {
	meta := &repoMeta{
		Templates: []repoMetaTemplate{
			{Name: "mlx-openwebui", Languages: []string{langSwift}, Targets: []string{targetDarwin}},
		},
		Languages: []repoMetaLanguage{
			{Key: langPython, Name: "Python"},
			{Key: langSwift, Name: "Swift"},
	...
	language, err := resolveTemplateLanguage(targetDarwin, "mlx-openwebui", meta, initOptions{})
	if err != nil {
	...
	_, err = resolveTemplateLanguage(targetDarwin, "mlx-openwebui", meta, initOptions{
		language:    langPython,
		languageSet: true,
	})
	if err == nil {
