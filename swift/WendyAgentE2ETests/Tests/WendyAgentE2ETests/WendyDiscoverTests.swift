...
struct `'wendy discover'` {
    ...
    func `finds WendyOS devices on the local network`() async throws {
        ...
        let record = try await self.cli.sh(
            "WENDY_ANALYTICS=false ./bin/wendy --json discover --timeout 1ms",
    ...
    func `'--json' formats devices as JSON`() async throws {
        try await self.cli.sh("WENDY_ANALYTICS=false ./bin/wendy --json discover --timeout 1ms") {
            standardOutput,
            standardError in
            #expect(standardError.isEmpty)
            let object = try Helper.jsonObject(from: standardOutput)
            #expect(
                object["usbDevices"] != nil || object["lanDevices"] != nil
                    || object["externalDevices"] != nil
            )
    ...
    func `reports clearly when no devices are found`() async throws {
        // TODO: Re-enable after adding the required fixture or implementation; one-by-one E2E run currently fails.
        let record = try await self.cli.sh(
            "WENDY_ANALYTICS=false ./bin/wendy discover --timeout 1ms --type lan",
            output: .string(limit: .max),
            error: .string(limit: .max)
        )
        #expect(record.terminationStatus.isSuccess)
        #expect(
            record.standardOutput?.contains("No devices found") == true
                || record.standardOutput?.contains("No WendyOS devices") == true
        )
    }
}
