...
struct `'wendy os'` {
    ...
    func `describes management subcommands`() async throws {
        try await self.cli.sh("./bin/wendy os --help") { standardOutput, standardError in
...
struct `'wendy os download'` {
    ...
    func `downloads a WendyOS image into the local cache`() async throws {
        ...
        let record = try await self.cli.sh(
            "\(Helper.commandEnvironment(home: home)) ./bin/wendy os download --version 1.0.0",
    ...
    func `reuses an already cached WendyOS image`() async throws {
        ...
        #expect(record.terminationStatus.isSuccess)
        #expect(
            record.standardOutput?.contains("already cached") == true
                || record.standardOutput?.contains("Using cached") == true
        )
    }
    ...
    func `fails clearly when the requested image is unavailable`() async throws {
        ...
        defer { try? FileManager.default.removeItem(at: home) }
        let record = try await self.cli.sh(
            "\(Helper.commandEnvironment(home: home)) ./bin/wendy os download --version 0.0.0-missing",
            output: .string(limit: .max),
            error: .string(limit: .max)
        )
        #expect(!record.terminationStatus.isSuccess)
        #expect(
            record.standardError?.contains("0.0.0-missing") == true
                || record.standardError?.contains("unavailable") == true
...
struct `'wendy os install'` {
    ...
    func `installs a WendyOS image onto the selected drive`() async throws {
        ...
        let record = try await self.cli.sh(
            "WENDY_ANALYTICS=false ./bin/wendy os install \(Helper.shellQuote(image.path)) /dev/disk-e2e --force --no-wifi",
            output: .string(limit: .max),
            error: .string(limit: .max)
        )
        #expect(record.terminationStatus.isSuccess)
        #expect(
            record.standardOutput?.contains("Installing") == true
                || record.standardOutput?.contains("Writing") == true
        )
    ...
    func `requires explicit confirmation before writing a drive`() async throws {
        ...
        let image = try Helper.writeFile("image", named: "wendyos.img", to: directory)
        let record = try await self.cli.sh(
            "printf 'n\n' | WENDY_ANALYTICS=false ./bin/wendy os install \(Helper.shellQuote(image.path)) /dev/disk-e2e --no-wifi",
            output: .string(limit: .max),
            error: .string(limit: .max)
        )
        #expect(
        ...
        #expect(
            record.standardOutput?.contains("Are you sure") == true
                || record.standardError?.contains("confirm") == true
                || record.standardError?.contains("force") == true
        )
    }
    ...
    func `fails clearly when the target drive is invalid`() async throws {
        ...
        let image = try Helper.writeFile("image", named: "wendyos.img", to: directory)
        let record = try await self.cli.sh(
            "WENDY_ANALYTICS=false ./bin/wendy os install \(Helper.shellQuote(image.path)) /definitely/not/a/drive --force --no-wifi",
            output: .string(limit: .max),
            error: .string(limit: .max)
        )
        #expect(!record.terminationStatus.isSuccess)
        #expect(
            record.standardError?.contains("drive") == true
...
struct `'wendy os list-drives'` {
    ...
    func `lists removable drives that can receive WendyOS`() async throws {
        try await self.cli.sh("WENDY_ANALYTICS=false ./bin/wendy os list-drives") {
            standardOutput,
            standardError in
            #expect(standardError.isEmpty)
            ...
            if trimmedOutput.hasPrefix("[") {
                ...
                #expect(
                    standardOutput.contains("Drive") || standardOutput.contains("No drives found")
                )
            }
        }
    }
    ...
    func `'--json' formats removable drives as JSON`() async throws {
        try await self.cli.sh("WENDY_ANALYTICS=false ./bin/wendy --json os list-drives") {
            standardOutput,
            standardError in
            #expect(standardError.isEmpty)
...
struct `'wendy os update'` {
    ...
    func `updates WendyOS on the selected device`() async throws {
        ...
        let record = try await self.cli.sh(
            "WENDY_ANALYTICS=false ./bin/wendy --device 127.0.0.1 os update \(Helper.shellQuote(artifact.path))",
            output: .string(limit: .max),
            error: .string(limit: .max)
        )
        #expect(record.terminationStatus.isSuccess)
        ...
        #expect(
            record.standardOutput?.contains("complete") == true
                || record.standardOutput?.contains("reboot") == true
        )
    }
    ...
    func `fails clearly when the selected device cannot be updated`() async throws {
        let record = try await self.cli.sh(
            "WENDY_ANALYTICS=false ./bin/wendy --device 127.0.0.1 os update --artifact-url http://127.0.0.1:9/missing.mender",
            output: .string(limit: .max),
            error: .string(limit: .max)
        )
        #expect(!record.terminationStatus.isSuccess)
