...
struct `'wendy run'` {
    ...
    func `builds and deploys the current project to the selected device`() async throws {
        ...
        try await self.cli.sh(
            "cd \(Helper.shellQuote(directory.path)) && WENDY_ANALYTICS=false \(Helper.shellQuote(wendy)) init --app-id sh.wendy.e2e.run --target wendyos --language swift --no-extra-entitlements --assistant skip --git-init no"
        ) { _, standardError in
        ...
        #expect(
            record.standardOutput?.contains("started") == true
                || record.standardOutput?.contains("deployed") == true
        )
    }
    ...
    func `streams deployment progress in a readable format`() async throws {
        ...
        let wendy = Helper.repositoryRootDirectoryURL().appendingPathComponent("go/bin/wendy").path
        try await self.cli.sh(
            "cd \(Helper.shellQuote(directory.path)) && WENDY_ANALYTICS=false \(Helper.shellQuote(wendy)) init --app-id sh.wendy.e2e.progress --target wendyos --language swift --no-extra-entitlements --assistant skip --git-init no"
        ) { _, _ in }
        ...
        #expect(record.standardOutput?.contains("Building") == true)
        #expect(
            record.standardOutput?.contains("Uploading") == true
                || record.standardOutput?.contains("Deploying") == true
        )
    ...
    func `'--json' formats deployment result as JSON`() async throws {
        ...
        let wendy = Helper.repositoryRootDirectoryURL().appendingPathComponent("go/bin/wendy").path
        try await self.cli.sh(
            "cd \(Helper.shellQuote(directory.path)) && WENDY_ANALYTICS=false \(Helper.shellQuote(wendy)) init --app-id sh.wendy.e2e.run-json --target wendyos --language swift --no-extra-entitlements --assistant skip --git-init no"
        ) { _, _ in }
        ...
        #expect(
            object["status"] as? String == "deployed" || object["status"] as? String == "running"
        )
    }
}
