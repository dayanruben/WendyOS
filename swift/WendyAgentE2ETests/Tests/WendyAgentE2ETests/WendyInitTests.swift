...
struct `wendy init` {
    ...
    func `creates a wendy json file for a new project`() async throws {
        ...
        #expect(FileManager.default.fileExists(atPath: directory.appendingPathComponent("Package.swift").path))
    }
    ...
    func `refuses to overwrite an existing wendy json file`() async throws {
        ...
        defer { try? FileManager.default.removeItem(at: directory) }
        let original = try Helper.writeWendyJSON(Helper.wendyJSONContents(appId: "sh.wendy.e2e.existing"), to: directory)
    ...
    func `validates project metadata before writing configuration`() async throws {
        ...
        #expect(record.standardError?.contains("invalid") == true || record.standardError?.contains("must") == true)
