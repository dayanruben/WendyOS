"""Motion-disabled-first live-state service for the exact u2525 adapter."""
...
REFERENCE_SHA256 = "98f12d860335bbaf6e770d626c3fd0efc6bfeff7cb9aad6cae36e296020b41b4"
CHECKPOINT_SHA256 = "4ed08c51b058ab6447770b36256ed1e0d450946435520ea67edbee888730a353"
COMMAND_POLICY_INDICES = (*range(12, 22), *range(29, 43))
ENTRY_POSITION_LIMIT_RAD = 0.03
ENTRY_SPEED_LIMIT_RAD_S = 0.03
...
ENTRY_HAND_SPEED_LIMIT_RAD_S = 0.15
...
ENTRY_MAXIMUM_STEP_RAD = 0.02
ENTRY_TRACKING_ABORT_RAD = float(
    os.environ.get("G1_ENTRY_TRACKING_ABORT_RAD", "0.10")
)
if not 0.03 <= ENTRY_TRACKING_ABORT_RAD <= 0.13:
    raise ValueError("G1_ENTRY_TRACKING_ABORT_RAD must be between 0.03 and 0.13")
...
class PhysicalProbeRuntime:
    ...
    def run_integrated_policy(
        self, command_id: str, *, maximum_policy_steps: int
    ) -> dict:
        ...
        return self.policy_runner.run(
            command_id, maximum_policy_steps=maximum_policy_steps
        )
...
def make_server(runtime: PhysicalProbeRuntime, host: str = "0.0.0.0", port: int = PORT):
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self):
            if self.path.split("?", 1)[0] not in ("/state", "/health"):
